...
export default function MenuManager() {
  ...
  const fetchMenuData = async () => {
    try {
      ...
      if (restaurantData.success) {
        const rest = restaurantData.data.restaurant;
        setRestaurant({
          _id: rest.id,
          name: rest.name,
          logo: rest.logo || "",
          ...
          isOpen: rest.status === "ACTIVE",
          phone: rest.phone || "",
          address: rest.address || "",
          openingHours: rest.openingHours || "Mon-Fri: 10:00-22:00",
        });
      }
      ...
      if (menuData.success) {
        ...
        setProducts(
          menu.products.map((prod: any) => ({
            ...
            image: prod.image || "",
            price: prod.price || 0,
            hasOptions: prod.hasOptions || false,
            hasAddons: prod.hasAddons || false,
        ...
        showAlert("success", "Menu loaded successfully");
      }
    } catch (err) {
  ...
  const openProductModal = (product: Product | null = null): void => {
    if (product) {
      ...
      setProductOptions(product.optionGroups || []);
      setProductAddons(product.addonGroups || []);
  ...
  const saveProduct = (): void => {
    ...
    if (currentProduct.hasOptions && productOptions.length > 0)
      productToSave.optionGroups = [...productOptions];
    if (currentProduct.hasAddons && productAddons.length > 0)
      productToSave.addonGroups = [...productAddons];
  ...
  const cloneProduct = (product: Product): void => {
    setProducts([
      ...products,
      { ...product, _id: `prod_${Date.now()}`, name: `${product.name} (copy)` },
    ]);
  ...
  const saveOptionGroup = (): void => {
    ...
    if (productOptions.some((og) => og._id === currentOptionGroup._id)) {
      setProductOptions(
        productOptions.map((og) =>
          og._id === currentOptionGroup._id ? currentOptionGroup : og
        )
      );
    } else {
  ...
  const deleteOptionGroup = (groupId: string): void => {
    setProductOptions(productOptions.filter((og) => og._id !== groupId));
  };
    ...
    setCurrentOptionGroup({
      ...
      items: [
        ...currentOptionGroup.items,
        {
          ...
          priceAdjustment: 0,
          isDefault: false,
          order: currentOptionGroup.items.length,
        },
      ],
    });
  };
  ...
  const saveAddonGroup = (): void => {
    ...
    if (productAddons.some((ag) => ag._id === currentAddonGroup._id)) {
      setProductAddons(
        productAddons.map((ag) =>
          ag._id === currentAddonGroup._id ? currentAddonGroup : ag
        )
      );
    } else {
  ...
  const deleteAddonGroup = (groupId: string): void => {
    setProductAddons(productAddons.filter((ag) => ag._id !== groupId));
  };
    ...
    setCurrentAddonGroup({
      ...
      items: [
        ...currentAddonGroup.items,
        {
          ...
          name: "",
          priceAdjustment: 0,
          order: currentAddonGroup.items.length,
        },
      ],
    });
  };
  ...
  const saveMenu = async (): Promise<void> => {
    ...
    try {
      ...
      // Build menu structure for backend
      const menuData = {
        restaurant: {
      ...
      if (data.success) {
        showAlert(
          "success",
          `✅ Menu saved!  ${data.data.stats.categories} categories, ${data.data.stats.products} products, ${data.data.stats.promotions} promotions`
        );
      } else {
        showAlert("error", data.error || "Failed to save menu");
      }
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {categories
                ...
                .map((cat) => (
                  <div
                    key={cat._id}
                    className="backdrop-blur-xl bg-white/5 rounded-2xl p-6 border border-white/10 hover:border-[#FFCE18]/30 transition-all"
                  >
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {promotions.map((promo) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-lg font-black text-[#FFCE18]">
        ...
        <div className="flex justify-end gap-4">
          <button
            onClick={() => router.push("/restaurant-admin/dashboard")}
            disabled={saving}
            className="px-8 py-4 rounded-xl bg-white/10 text-white font-bold hover:bg-white/20 transition-all disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={saveMenu}
            disabled={saving}
            className="px-12 py-4 rounded-xl bg-gradient-to-r from-[#FFCE18] to-[#ffd94d] text-black font-black text-lg hover:scale-105 transition-all flex items-center gap-3 shadow-2xl disabled:opacity-50"
          >
            {saving ? (
              <>
                        ...
                        >
                          <div className="flex items-start justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() => deleteOptionGroup(og._id)}
                        ...
                        >
                          <div className="flex items-start justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() => deleteAddonGroup(ag._id)}
                                className="p-2 rounded-lg bg-red-500/20 text-red-400"
                              >
                                <FaTrash />
                              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gradient-to-b from-gray-900 to-black rounded-3xl border border-white/10 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-gray-900/90 backdrop-blur-xl p-6 border-b border-white/10 flex items-center justify-between">
              <h3 className="text-2xl font-black text-white">
                {productOptions.some((og) => og._id === currentOptionGroup?._id)
                  ? "Edit Option Group"
                  : "Add Option Group"}
              </h3>
              <button
      ...
      {showAddonGroupModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gradient-to-b from-gray-900 to-black rounded-3xl border border-white/10 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-gray-900/90 backdrop-blur-xl p-6 border-b border-white/10 flex items-center justify-between">
              <h3 className="text-2xl font-black text-white">
                {productAddons.some((ag) => ag._id === currentAddonGroup?._id)
                  ? "Edit Addon Group"
                  : "Add Addon Group"}
              </h3>
              <button
